...
#传递核心参数
parser = argparse.ArgumentParser(description='config_num')
parser.add_argument('--config_num', type=int, default='1', help='config_num')
...
LOGGING_CONFIG = {
    ...
    "handlers": {
        ...
        "file": {
            ...
            "level": "DEBUG",
            "formatter": "simple",
            "filename": f"{os.getcwd()}/logs/ChaiRosettaScore_{args.config_num}.log",
            "encoding": "utf8",
            "mode": "a",
        }
    },
...
os.chdir("/home/users/hcdai/AI-peptide/ChaiRosettaScore_par")
# 读取配置文件
with open(f'config/config{args.config_num}.json', 'r') as f:
    config = json.load(f)
...
scores = np.load(output_dir.joinpath("scores.model_idx_2.npz"))'''  
    ...
    run_file = Path(os.path.join(chaiworkdir, f"run{args.config_num}.py"))
...
def chai_rosetta_score(receptor:dict, ligand_name:str, ligand_sequence:str, receptor_chain_num = parameter["receptor_chain_num"], path = path):
    ...
    print(pdb_names)
    pdb_names = os.path.join(pdb_path, pdb_names[0])
    print(pdb_names)
    # rosetta_score(pdb_path, score_output_dir, rosetta_path)
    rosetta_score(pdb_names, score_output_dir, rosetta_path)
    ...
    try:
        ...
        os.remove(f"/home/users/hcdai/AI-peptide/Chai-1/chai-lab/RMSD_Time-test/run{args.config_num}.py")
        print(f"Cleaned up run{args.config_num}.py file.")
    except:
        pass  
...
while True:
    try:
        try:
            ...
            print("Cleaned up temp directory.")
            os.remove(f"/home/users/hcdai/AI-peptide/Chai-1/chai-lab/RMSD_Time-test/run{args.config_num}.py")
            logging.info(f"Cleaned up run{args.config_num}.py file.")
            print(f"Cleaned up run{args.config_num}.py file.")
        except:
