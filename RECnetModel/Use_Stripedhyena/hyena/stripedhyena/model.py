...
from stripedhyena.ESMCembeding import ESMCembeding, ESMC_600M_202412, ESMCtokenizer
...
acc_or_not = True
ESMC_model = ESMC_600M_202412(device = "cuda", use_flash_attn= acc_or_not,model_path = ESMC_path)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, u, inference_params=None, padding_mask=None, *args, **kwargs):
        ...
        dtype_mha = self.inner_mha_cls.Wqkv.weight.dtype  # 获取 MHA 参数类型
        u = u.to(dtype_mha)  # 统一类型
        
        u = (
            self.inner_mha_cls(
                self.pre_norm(u).to(dtype_mha),
...
class StripedHyena(nn.Module):
    def __init__(self, config):
        ...
        self.embedding_layer = ESMCembeding(model = ESMC_model, acc_or_not = acc_or_not, device ='cuda')
        ...
        self.tokenizer = ESMCtokenizer(acc_or_not, model = ESMC_model)
